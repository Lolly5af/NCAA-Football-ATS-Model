...
def run_backtest(df,start_season,end_season):
    ...
    settled=pred.dropna(subset=["correct","y"]).copy(); rows=[summary_row(g,s) for s,g in settled.groupby("season")]; rows.append(summary_row(settled,"ALL")); summary=pd.DataFrame(rows)
    Path("reports").mkdir(exist_ok=True); summary.to_csv("reports/backtest_summary.csv",index=False); pred.to_csv("reports/backtest_predictions.csv",index=False); print(summary.to_string(index=False)); return summary,pred

def summary_row(g,label):
    # Baseline is always the sportsbook favorite: home if spread < 0, otherwise away.
    ...
    return {"season":label,"games":len(g),"ats_accuracy":g.correct.mean(),"market_favorite_baseline":np.nanmean(market_correct),"model_minus_market":g.correct.mean()-np.nanmean(market_correct),"brier":brier_score_loss(g.y,g.p_home_cover),"log_loss":log_loss(g.y,g.p_home_cover,labels=[0,1]),"avg_abs_spread_edge":np.mean(np.abs(g.model_margin+g.spread))}
